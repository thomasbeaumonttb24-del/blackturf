    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {/* ── Dutch bet ── */}
          {analysis?.dutch_bet?.is_profitable && (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 space-y-3">
              ...
              <p className="text-xs text-gray-500">{analysis.dutch_bet.note}</p>
              <div className="grid gap-1.5">
                {analysis.dutch_bet.mises.map((m, i) => (
                  <div key={i} className="flex items-center justify-between text-xs bg-white rounded-lg px-3 py-2 border border-emerald-100">
                    <span className="font-medium">N°{m.numero} {m.nom}</span>
                    <span className="text-gray-500">Cote {m.cote.toFixed(1)}</span>
                    <span className="font-mono font-bold">{m.mise.toFixed(2)}€</span>
                    <span className="text-emerald-600 font-semibold">+{m.profit_si_gagne.toFixed(2)}€</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* ── Paris multiples (combinaisons, probabilités simulées) ── */}
          {analysis?.paris_multiples && analysis.paris_multiples.proposals.length > 0 && (
            <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
              <div className="flex items-center gap-2">
                <Calculator className="h-4 w-4 text-brand-gold" />
                <h3 className="text-sm font-semibold text-gray-900">Paris multiples conseillés</h3>
                <span className="ml-auto text-[10px] text-gray-400">
                  {analysis.paris_multiples.simulations.toLocaleString("fr-FR")} simulations
                </span>
              </div>

              {/* Scénario d'arrivée le plus probable */}
              {analysis.paris_multiples.scenario_arrivee.length > 0 && (
                <div className="rounded-lg bg-gray-50 p-2.5">
                  <p className="text-[11px] font-semibold text-gray-500 mb-1.5">Arrivée la plus probable (modèle)</p>
                  <div className="flex flex-wrap items-center gap-1.5">
                    {analysis.paris_multiples.scenario_arrivee.map((h, i) => (
                      <span key={h.numero} className="inline-flex items-center gap-1 rounded-md bg-white ring-1 ring-gray-200 px-1.5 py-0.5 text-xs">
                        <span className="font-bold text-gray-400 tabular-nums">{i + 1}</span>
                        <span className="font-semibold">N°{h.numero}</span>
                        <span className="text-gray-400 truncate max-w-[90px]">{h.nom}</span>
                        <span className="text-[10px] text-brand-gold font-medium tabular-nums">{Math.round(h.proba_victoire * 100)}%</span>
                      </span>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {analysis.paris_multiples.proposals.map((p, i) => {
                  const nivColor = p.niveau === "safe" ? "emerald" : p.niveau === "equilibre" ? "blue" : p.niveau === "audacieux" ? "amber" : "red";
                  const nivLabel = p.niveau === "safe" ? "Sécurité" : p.niveau === "equilibre" ? "Équilibré" : p.niveau === "audacieux" ? "Audacieux" : "Gros lot";
                  return (
                    <div key={i} className={cn(
                      "rounded-xl border p-3 space-y-2",
                      nivColor === "emerald" ? "border-emerald-200 bg-emerald-50/40" :
                      nivColor === "blue" ? "border-blue-200 bg-blue-50/40" :
                      nivColor === "amber" ? "border-amber-200 bg-amber-50/40" : "border-red-200 bg-red-50/40",
                    )}>
                      <div className="flex items-center gap-2">
                        <span className={cn("text-[9px] font-bold uppercase tracking-wide rounded-full px-1.5 py-0.5",
                          nivColor === "emerald" ? "bg-emerald-100 text-emerald-700" :
                          nivColor === "blue" ? "bg-blue-100 text-blue-700" :
                          nivColor === "amber" ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700")}>
                          {nivLabel}
                        </span>
                        <span className="text-sm font-bold text-gray-900">{p.type_pari}</span>
                        <span className={cn("ml-auto text-xs font-bold tabular-nums",
                          p.ev > 0 ? "text-emerald-600" : "text-gray-400")}>
                          {p.ev > 0 ? "+" : ""}{Math.round(p.ev * 100)}% espérance
                        </span>
                      </div>
                      {/* Chevaux */}
                      <div className="flex flex-wrap gap-1">
                        {p.chevaux.map((c) => (
                          <span key={c.numero} className="inline-flex items-center gap-1 rounded-md bg-white ring-1 ring-gray-200 px-1.5 py-0.5 text-[11px]">
                            <span className="font-bold">N°{c.numero}</span>
                            <span className="text-gray-400 truncate max-w-[70px]">{c.nom}</span>
                          </span>
                        ))}
                      </div>
                      {/* Métriques */}
                      <div className="grid grid-cols-3 gap-1 text-center">
                        <div className="rounded-lg bg-white/70 py-1">
                          <p className="text-[9px] text-gray-400 uppercase">Proba</p>
                          <p className="text-xs font-bold tabular-nums">{(p.proba_gain * 100).toFixed(p.proba_gain < 0.1 ? 1 : 0)}%</p>
                        </div>
                        <div className="rounded-lg bg-white/70 py-1">
                          <p className="text-[9px] text-gray-400 uppercase">Rapport</p>
                          <p className="text-xs font-bold tabular-nums">×{p.rapport_estime.toFixed(1)}</p>
                        </div>
                        <div className="rounded-lg bg-white/70 py-1">
                          <p className="text-[9px] text-gray-400 uppercase">Mise</p>
                          <p className="text-xs font-bold tabular-nums">{p.cout_total.toFixed(0)}€</p>
                        </div>
                      </div>
                      <p className="text-[11px] text-gray-500 leading-snug">{p.texte_explication}</p>
                      <div className="flex items-center justify-between text-[10px]">
                        <span className="text-gray-400">Gain potentiel <strong className="text-gray-700 tabular-nums">{p.gain_potentiel.toFixed(0)}€</strong></span>
                        {p.edge > 0.01 && (
                          <span className="text-emerald-600 font-medium">Avantage modèle +{Math.round(p.edge * 100)}%</span>
                        )}
                      </div>
                    </div>
                  );
              ...
              <p className="text-[10px] text-gray-400 leading-snug">
                Probabilités estimées par simulation de {analysis.paris_multiples.simulations.toLocaleString("fr-FR")} courses (modèle Plackett-Luce).
                Rapports estimés selon le marché — les gains réels dépendent des mises finales. Jouez avec modération.
              </p>
            </div>
          )}
