...
const TYPE_CONFIG: Record<string, { icon: React.ElementType; color: string; bg: string; label: string; link: (id: string) => string }> = {
  cheval:     { icon: Search, color: "text-amber-600",  bg: "bg-amber-50",  label: "Chevaux",     link: (id) => `/chevaux/${id}` },
  jockey:     { icon: User,   color: "text-blue-600",   bg: "bg-blue-50",   label: "Jockeys",     link: (id) => `/jockeys/${id}` },
  hippodrome: { icon: MapPin, color: "text-green-600",  bg: "bg-green-50",  label: "Hippodromes", link: (id) => `/programme?hippodrome=${encodeURIComponent(id)}` },
  course:     { icon: Clock,  color: "text-purple-600", bg: "bg-purple-50", label: "Courses",     link: (id) => `/courses/${id}` },
};
            ...
            return (
              <div key={type}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs text-gray-400">{items.length} résultat{items.length > 1 ? "s" : ""}</span>
                </div>
                  ...
                  {items.map((r) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400 truncate mt-0.5">{r.sub}</p>
                      </div>
