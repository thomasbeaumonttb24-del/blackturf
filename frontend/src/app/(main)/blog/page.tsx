...
export default function BlogIndex() {
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd(filAriane([{ nom: "Accueil", url: "/" }, { nom: "Blog" }])) }} />
      ...
      <Container>
          ...
          >
            <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wider text-brand-gold-dark">
              ...
              {featured.tags.slice(0, 1).map((t) => (
                <span key={t} className="text-brand-charcoal">{t}</span>
              ))}
            </div>
            <h2 className="mt-3 font-display text-2xl font-bold tracking-tight text-brand-dark transition-colors group-hover:text-brand-gold-dark sm:text-3xl">
              {featured.title}
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-brand-charcoal sm:text-base">
              {featured.description}
            </p>
            <div className="mt-4 flex items-center gap-3 text-xs text-brand-charcoal">
              ...
              <span className="inline-flex items-center gap-1 font-medium text-brand-gold-dark">
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2">
          {rest.map((a) => (
            ...
            >
              <div className="flex flex-wrap items-center gap-2 text-[11px] text-brand-charcoal">
              ...
              <h3 className="mt-2 font-display text-lg font-semibold leading-snug text-brand-dark transition-colors group-hover:text-brand-gold-dark">
                {a.title}
              </h3>
              <p className="mt-1.5 flex-1 text-sm leading-relaxed text-brand-charcoal">{a.description}</p>
