      ...
      <Container>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {d.points.map((p) => (
              <div key={p} className="flex items-start gap-3 rounded-xl border border-gray-200 bg-white p-4">
                ...
                <span className="text-sm leading-relaxed text-brand-charcoal/80">{p}</span>
              </div>
            ))}
          </div>
        </Section>
          ...
          {courses.length > 0 ? (
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-gray-50/50 px-4 py-5 text-sm text-brand-charcoal/70">
              ...
              <span>
                ...
                <Link href="/programme" className="font-medium text-brand-gold-deep underline">programme complet</Link>.
              </span>
            </div>
          )}
        </Section>
        ...
        <Section title="Pour aller plus loin">
          <p className="text-sm leading-relaxed text-brand-charcoal/80">
            {d.slug === "trot" ? (
              <>
                Voir nos guides : <Link href="/blog/strategies-paris-trot" className="font-medium text-brand-gold-deep underline">5 clés pour parier au trot</Link>{" "}
                et <Link href="/blog/reduction-kilometrique-trot" className="font-medium text-brand-gold-deep underline">la réduction kilométrique</Link>.
              </>
            ) : (
              <>
                Comprendre les <Link href="/guides/types-de-paris-pmu" className="font-medium text-brand-gold-deep underline">types de paris PMU</Link>{" "}
                et le <Link href="/guides/pari-de-valeur" className="font-medium text-brand-gold-deep underline">pari de valeur</Link>.
              </>
            )}
          </p>
        </Section>
