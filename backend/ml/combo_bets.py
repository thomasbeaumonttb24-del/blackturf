...
def enumerate_bet_candidates(
    ...
    # ── ANCRE FAVORI EXPLICITE (mode engagement « gagne souvent ») ──────────────
    # Garantit le Simple Placé du FAVORI MARCHÉ (cote la plus basse) pour le prudent, même
    # quand le sim sous-estime sa proba placé (le favori se place ~72% en réalité, le sim
    # le sous-cote → l'ancre par seuil ne se déclenchait pas). proba_gain plancher 0.55 →
    # forte conviction → sélectionné. Seul le prudent autorise le Simple Placé → zéro impact
    # sur modéré/risqué.
    fav_i = int(np.argmin(cotes))
    if ("Simple Placé", (fav_i,)) not in seen and float(cotes[fav_i]) <= 9.0:
        p_pl_m_f = max(float(sim_m.p_simple_place(fav_i)), 1e-3)
        rap_f = float(min(max(TRJ_PLACE / p_pl_m_f, 1.1), 50.0))
        if rap_f >= 1.05:
            p_aff_f = max(float(sim.p_simple_place(fav_i)), p_pl_m_f, 0.55)
            seen.add(("Simple Placé", (fav_i,)))
            cands.append({
                "niveau": "securite", "type_pari": "Simple Placé", "chevaux": [H(fav_i)],
                "proba_gain": round(p_aff_f, 4), "rapport_estime": round(rap_f, 1),
                "ev": round(p_aff_f * rap_f - 1.0, 3),
                "edge": round(float(p_aff_f - p_pl_m_f), 4), "_anchor": True,
                "texte_explication": (
                    f"N°{numeros[fav_i]} {noms[fav_i]} FAVORI — {p_aff_f*100:.0f}% d'être "
                    f"dans les 3 (cote {cotes[fav_i]:.1f}) : le placé qui tombe le plus souvent."
                ),
            })
