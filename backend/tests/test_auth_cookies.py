...
async def _inscrire(client: AsyncClient, email: str = "cookie@blackturf.fr"):
    return await client.post("/api/v1/auth/register", json={
        "email": email, "password": "MotDePasse123", "nom": "T", "prenom": "U",
    })
...
async def test_login_pose_des_cookies_httponly(client: AsyncClient):
    await _inscrire(client, "login-cookie@blackturf.fr")
    resp = await client.post("/api/v1/auth/login", data={
        "username": "login-cookie@blackturf.fr", "password": "MotDePasse123",
    })
...
async def test_le_temoin_de_session_est_lisible_et_ne_contient_aucun_secret(client: AsyncClient):
    ...
    resp = await _inscrire(client, "temoin@blackturf.fr")
    poses = _cookies_poses(resp)
...
async def test_le_cookie_seul_authentifie(client: AsyncClient):
    """Aucun en-tête Authorization : la requête ne doit tenir que par le cookie."""
    await _inscrire(client, "cookie-seul@blackturf.fr")
...
async def test_l_entete_authorization_reste_accepte(client: AsyncClient):
    """Clients non navigateur et onglets pas encore rechargés : l'en-tête vit encore."""
    resp = await _inscrire(client, "entete@blackturf.fr")
...
async def test_refresh_sans_corps_utilise_le_cookie(client: AsyncClient):
    """Le front ne peut plus lire le refresh token : il ne peut donc plus l'envoyer."""
    await _inscrire(client, "refresh-cookie@blackturf.fr")
    resp = await client.post("/api/v1/auth/refresh")
    assert resp.status_code == 200
    assert "access_token" in _cookies_poses(resp)


async def test_refresh_accepte_encore_le_corps_pour_la_bascule(client: AsyncClient):
    ...
    resp = await _inscrire(client, "bascule@blackturf.fr")
...
async def test_logout_efface_les_cookies(client: AsyncClient):
    """Un cookie httpOnly ne peut être effacé que par le serveur."""
    await _inscrire(client, "logout@blackturf.fr")
